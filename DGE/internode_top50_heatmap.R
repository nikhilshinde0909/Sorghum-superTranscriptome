...
data <- read.table('/home/mpilab/DEG_ST/internode_gene_cont.csv', header = T, sep = ',')
data <- data %>% arrange(-V,-FL, -RI, -BL, -ANT, -SD)
data <- data[1:50,]
top_genes <- data %>% select(1)
write.table(top_genes, '/home/mpilab/DEG_ST/sorghum_internode_top_genes.list', row.names = F, 
            col.names = T, sep = '\t', quote = F)

data1 <- read.table('/home/mpilab/DEG_ST/sorghum_internode_top_genes.TSV', 
                    header = T,  sep = '\t')
data1 <- data1 %>%
...
ggp1 
# save heatmap
tiff("/home/mpilab/DEG_ST//internode_top50_heatmap.tiff", units="cm", width = 25, height=30, res=300)
